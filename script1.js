...
async function getData(selected_product, received_quantity) {
    ...
    const { new_array_index, new_array_value, upperCIs, lowerCIs } = generateTrendline(selected_model);

    recommendation(received_quantity);
    plotGraphCompleteData(new_array_index, new_array_value, upperCIs, lowerCIs, xArrayPrev, yArrayPrev, xArrayCurr, yArrayCurr);
}
...
function generateTrendline(model) {
    ...
    //Finding Max ele index
    let max_index = 0;
    for (let i = 1; i < yArray.length; i++) {
        if (yValues[i] > yValues[max_index]) {
            max_index = i;
        }
    }
    console.log('Max Element Index :', max_index);

    //Finding Max ele value
    let max_value = Math.max(...yValues);
    console.log('Max Element Value :', max_value);

    new_array_index = xValues.slice(0, max_index + 1);
    console.log('new_array_index :', new_array_index);

    new_array_value = yValues.slice(0, max_index + 1);
    console.log('new_array_value :', new_array_value);

    return { new_array_index, new_array_value, upperCIs, lowerCIs };
}
...
function displayData() {
    ...
    for (const [name, array] of Object.entries(ucCode_arrays_SKU)) {
